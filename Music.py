...
class Music:
    ...
    async def summon(self, ctx):
        ...
        if summoned_channel is None:
            await self.bot.say('Are you sure you are in a channel? :thinking: ')
            return False
    ...
    async def volume(self, ctx, value : int):
        ...
        if state.is_playing():
            ...
            await self.bot.say('The volume has been set to {:.0%}'.format(player.volume))
    @commands.command(pass_context=True, no_pm=True)
    ...
    async def skip(self, ctx):
        ...
        if voter == state.current.requester:
            await self.bot.say('Requester skipped the song...')
            state.skip()
        elif voter.id not in state.skip_votes:
            state.skip_votes.add(voter.id)
            total_votes = len(state.skip_votes)
            if total_votes >= 1:
                await self.bot.say('The song has been skipped...')
                state.skip()
            else:
